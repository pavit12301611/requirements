...
// ---- status visibility -------------------------------------------------
// live project → 200
const livePub = await req('/api/public/daily-bloom');
...
const closedSubmit = await req('/api/public/photo-portfolio/submit', {
...
const okSubmit = await req('/api/public/daily-bloom/submit', {
  method: 'POST',
  body: {
...
const subs = await req(`/api/projects/${draftProj.id}`, { cookie }); // sanity: still reachable
const liveProj = projects.json.projects.find(p => p.slug === 'daily-bloom');
const liveSubs = await req(`/api/projects/${liveProj.id}/submissions`, { cookie });
const found = liveSubs.json.submissions.some(s => s.customer_name === 'Test Client');
check('submission visible to admin', found);

// invalid email rejected
const badEmail = await req('/api/public/daily-bloom/submit', {
  method: 'POST',
